...
def network_trainer(
        data_loaders: Tuple[List[Tuple[str, data.DataLoader]], List[Tuple[str, data.DataLoader]]],  # from fetch_data
        model: nn.Module,  # model s3n in sss_net.py
        criterion: object,
    ...
    def process(split, data_loader, is_train):
        ...
        for batch_idx, (input, target) in enumerate(progress_bar(data_loader, ascii=True, desc=split, unit='batch', leave=False)):
            ...
            if not meters is None:
                ctx.metrics.update({
                    split + '_' + k: v(ctx) for k, v in meters.items() if v is not None
                })
